...
export default function render(data, coords, lang) {
    // Rendering city map
    document.getElementById('map__img').src = `https://maps.google.com/maps?&q=${data.location.name}&t=&z=11&ie=UTF8&iwloc=&hl=${lang}&output=embed`
    // Hiding loader and displaying the page
    document.querySelector('.loader').style.display = "none"
    document.querySelector('.wrapper').style.display = "flex"
    // Getting and updating other elements
    ...
    document.getElementById('map__coords_lat').textContent = parseFloat(coords.lat).toFixed(7)
    document.getElementById('map__coords_long').textContent = parseFloat(coords.long).toFixed(7)
